...
#include "core/draw.h"     /* primitives de dessin portables */
#include "core/scopes.h"   /* rendu portable des scopes (CoreScope/CoreParams) */
...
/* CHROMA_MAX (echelle du cercle 100% du vectorscope) est defini dans core/scopes.c
   et declare dans core/scopes.h ; TplVec le reutilise via l'en-tete. */
...
{
    CoreParams p;
    p.sR=g_sR; p.sG=g_sG; p.sB=g_sB; p.sCol=g_sCol;
...
{
    CoreScope cs;
    cs.bits=g_sc[idx].bits; cs.tpl=g_sc[idx].tpl;
    cs.w=g_sc[idx].w; cs.h=g_sc[idx].h; cs.on=g_sc[idx].on;
    return cs;
}

static void RenderVec(void)
{
    CoreScope cs=CoreScopeOf(SC_VEC);
...
static void RenderWaveLike(int idx,BOOL parade)
{
    ...
    CoreParams p=CoreParamsNow();
    CoreRenderWaveLike(&cs,&p,parade?1:0);
}

static void RenderHist(void)
{
    CoreScope cs=CoreScopeOf(SC_HIST);
    CoreParams p=CoreParamsNow();
    CoreRenderHist(&cs,&p);
}
